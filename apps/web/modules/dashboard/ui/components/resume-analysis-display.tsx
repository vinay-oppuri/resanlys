    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out pb-8">
            <div className="space-y-2 border-b border-border/40 pb-6">
                <h2 className="text-3xl font-bold tracking-tighter">Analysis Report</h2>
                <p className="text-muted-foreground text-lg">AI-powered insights to optimize your resume.</p>
            </div>
            ...
            {data.overall_match_feedback && (
                <div className="p-6 rounded-2xl bg-linear-to-br from-primary/5 to-transparent border border-primary/10 shadow-sm">
                    <h3 className="text-lg font-semibold mb-3 flex items-center gap-2 text-primary">
                        <span className="text-xl">💡</span> Overall Verdict
                    </h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2.5">
                            {data.missing_keywords.map((keyword, i) => (
                                <span key={i} className="px-3.5 py-1.5 rounded-full bg-red-500/5 text-red-600 dark:text-red-400 text-sm font-semibold border border-red-500/10 hover:bg-red-500/10 transition-colors cursor-default">
                                    {keyword}
                                </span>
                            ))}
                        </div>
                            ...
                            {data.weak_or_underrepresented_skills.map((item, i) => {
                                ...
                                return (
                                    <div key={i} className="flex flex-col gap-2 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 hover:bg-amber-500/10 transition-colors">
                                        ...
                                        {suggestion && <span className="text-sm text-amber-900/70 dark:text-amber-200/70 pl-3.5 italic">{suggestion}</span>}
                                    </div>
                                )
                            })}
                        </div>
            ...
            {data.bullet_point_improvements?.length > 0 && (
                <div className="space-y-6 pt-6 border-t border-border/40">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xl font-bold tracking-tight">Suggested Refinements</h3>
                        ...
                        {data.bullet_point_improvements.map((item, i) => (
                            <div key={i} className="flex flex-col gap-4 p-6 rounded-2xl border border-border/40 bg-card/40 hover:bg-card/80 hover:shadow-lg transition-all duration-300 group">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-muted-foreground/60 line-through decoration-red-500/30 leading-relaxed font-serif italic pl-2 border-l-2 border-red-500/10">
                                        {item.original}
                                    </p>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <p className="text-base font-medium text-foreground leading-relaxed pl-2 border-l-2 border-green-500/20 group-hover:border-green-500/50 transition-colors">
                                        {item.improved}
                                    </p>
                                </div>
                            </div>
